...
function clearAppModuleCache() {
  for (const filename of Object.keys(require.cache)) {
    if (filename.startsWith(path.join(srcDir, "app", "api")) || filename.startsWith(path.join(srcDir, "lib", "image"))) {
...
require(path.join(testsDir, "image-api.test.ts"));
...
(async () => {
  ...
  if (failed > 0) {
    process.exitCode = 1;
    console.error(`Image API tests failed: ${failed}`);
  } else {
    console.log(`Image API tests passed: ${tests.length}`);
  }
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
